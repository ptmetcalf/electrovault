...
Example:
  python scripts/test_caption_prompt.py /absolute/path/to/photo.jpg --show-prompt
"""
...
from photo_brain.vision.captioner import _build_caption_prompt, describe_photo
from photo_brain.vision.model_client import LocalModelError
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional context string forwarded to the captioner prompt.",
    )
    parser.add_argument(
        "--show-prompt",
        action="store_true",
    ...
    prompt = _build_caption_prompt(args.context)
    if args.show_prompt:
        print("=== Captioner prompt ===")
        print(prompt)
        print()

    photo = _build_photo_model(image_path)
    try:
        vision_desc = describe_photo(photo, context=args.context)
